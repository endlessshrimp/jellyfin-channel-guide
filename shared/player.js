...
(() => {
    ...
    // #/weather, #/rooms, #/cameras, the hubs (#/sports, #/news) and #/books
    // are HOMER's own pages (Jellyfin has none of them), so they always count
    const isWeatherHash = (h) => /^#\/(weather|rooms|cameras|sports|news|books)(\?|$)/.test(h);
