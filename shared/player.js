...
(() => {
    ...
    // #/weather, #/rooms, the hubs (#/sports, #/news), #/books and #/music are
    // HOMER's own pages (Jellyfin has none of them), so they always count
    const isWeatherHash = (h) => /^#\/(weather|rooms|sports|news|books|music)(\?|$)/.test(h);
